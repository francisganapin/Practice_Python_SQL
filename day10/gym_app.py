...
from model.models import GymMember 
...
@app.route('/member/expiry',methods=['POST'])
def get_member_expiry_1():
    data = request.get_json()
    key_code = data.get('key_code', "")
    with Session(engine) as session:
        statement = select(GymMember).where(GymMember.key_code == key_code)
...
@app.route('/member/expiry/<key_code>',methods=['GET'])
def get_member_expiry_2(key_code):
    with Session(engine) as session:
        statement = select(GymMember).where(GymMember.key_code == key_code)
        member = session.exec(statement).first()
        if member is None:
